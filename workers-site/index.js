...
async function handleEvent(event) {
    ...
    let options = {}
    console.log("first")
    const requestURL = event.request.url.replaceAll('%3A', ":").replaceAll('%2F', '/')
    const requestPath = requestURL.replaceAll(/https:\/\/.*?\//gi, '')
    if (requestURL.endsWith('png')) return serveImage(requestPath)
    if (requestPath.startsWith('shein/info')) return getProductInfo(requestURL)
    console.log("oops")
        /**
         * You can add custom logic to how we fetch your assets
         * by configuring the function `mapRequestToAsset`
         */
        // options.mapRequestToAsset = handlePrefix(/^\/trello/)
...
async function serveImage(key) {
    ...
    if (object === null) {
        return new Response('Object Not Found: s' + key, { status: 404 });
    }
